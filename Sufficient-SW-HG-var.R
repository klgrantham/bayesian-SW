...
sig_sq_cluster <- 0.08  # between-cluster variance
sig_sq_cp <- 0.02       # between-cluster-period variance
...
# implied within-period intracluster correlation (WPICC)
WPICC <- (sig_sq_cluster + sig_sq_cp) / (sig_sq_cluster + sig_sq_cp + sig_sq_subject)

# implied between-period intracluster correlation (BPICC)
BPICC <- sig_sq_cluster / (sig_sq_cluster + sig_sq_cp + sig_sq_subject)

# implied cluster autocorrelation (CAC)
CAC <- BPICC/WPICC
...
fit <- stan(file='Sufficient-SW-HG-var.stan', data = data, iter = 11e3, warmup = 1e3, thin=2)

# ---- get diagnostics ----

pars <- c('theta', 'sig_sq_cluster', 'sig_sq_cp', 'sig_sq_subject', 'WPICC', 'CAC', 'C')
...
true.vals <- c(theta, sig_sq_cluster, sig_sq_cp, sig_sq_subject, WPICC, CAC, C)
