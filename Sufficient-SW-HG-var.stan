// Stan program for Sufficient-SW-HG model
data {
  int cluster_periods;
  int clusters;
  int periods;
...
parameters {
  ...
  real<lower=0> sig_sq_cp;       // between-cluster-period variance
}
transformed parameters {
  // we also want draws for our implied within-cluster correlations
  real<lower=0,upper=1> WPICC = (sig_sq_cluster + sig_sq_cp) / (sig_sq_cluster + sig_sq_cp + sig_sq_subject);
  real<lower=0,upper=1> CAC = sig_sq_cluster / (sig_sq_cluster + sig_sq_cp);
}
model {
  ...
  sig_sq_cluster ~ inv_gamma(2, 0.5);
  sig_sq_cp ~ inv_gamma(2, 0.5);
