...
plot_prior <- function(df) {
  p <- ggplot(df) +
        geom_area(aes(x=x, y=y), color="darkorange", fill="darkorange", alpha=0.6) +
        ylab("") +
        theme_bw() +
...
plot_dens <- function(densvals) {
  p <- ggplot() +
        geom_density(aes(x=densvals), fill="slateblue", alpha=0.6) +
        ylab("") +
        theme_bw() +
        theme(plot.title = element_text(hjust = 0.5, size=20),
              axis.title=element_text(size=18),
...
plot_prior(theta_prior) +
  geom_vline(aes(xintercept=0), color="steelblue", size=1) +
  xlab(expression(theta)) +
  ggtitle(expression(paste(theta, " ~ N(0, 10000)")))
...
plot_prior(WPICC_prior) +
  geom_vline(aes(xintercept=0.05), color="steelblue", size=1) +
  geom_vline(aes(xintercept=0.1), color="darkseagreen", size=1) +
...
plot_prior(CAC_prior) +
  geom_vline(aes(xintercept=0.8), color="steelblue", size=1) +
  xlab(expression(r)) +
...
plot_prior(sig2e_prior) +
  geom_vline(aes(xintercept=1.0), color="steelblue", size=1) +
...
plot_dens(sig2c_prior) +
  geom_vline(aes(xintercept=sig_sq_c1), color="steelblue", size=1) +
  geom_vline(aes(xintercept=sig_sq_c2), color="darkseagreen", size=1) +
...
plot_dens(sig2cp_prior) +
  geom_vline(aes(xintercept=sig_sq_cp1), color="steelblue", size=1) +
  geom_vline(aes(xintercept=sig_sq_cp2), color="darkseagreen", size=1) +
